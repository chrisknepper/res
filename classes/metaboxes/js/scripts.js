jQuery(function($) {
	...
	$('.meta_box_repeatable_add').live('click', function() {
		...
		clone.find('input, textarea, select')
			.attr('name', function(index, name) {
				return name.replace(/\[([0-9]+)\]/, function(fullMatch, n) {
					newNumber = parseInt(n, 10) + 1;
					return '[' + newNumber + ']';
				});
			});
		var arr = [];
	...
	window.initMetaboxSortables = function() {
		$('.meta_box_repeatable tbody').sortable({
			...
			update: function(event, ui) {
				...
				$('.meta_box_repeatable .ui-sortable tr').each(function(outerIndex, element) {
					$(element).find('input, textarea, select')
						.attr('name', function(innerIndex, name) {
							return name.replace(/\[([0-9]+)\]/, function(fullMatch, n) {
								return '[' + (outerIndex - 1) + ']';
							});
						});
